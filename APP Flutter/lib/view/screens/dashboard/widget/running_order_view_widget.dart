        ...
        child: Column(children: [
          ...
          ListView.builder(
              ...
              itemBuilder: (context, index){
                ...
                if(orderStatus == OrderStatusType.pending.name){
                  status = 1;
                }else if(orderStatus == OrderStatusType.accepted.name || orderStatus == OrderStatusType.processing.name || orderStatus == OrderStatusType.confirmed.name){
                  status = 2;
                }else if(orderStatus == OrderStatusType.handover.name || orderStatus == OrderStatusType.picked_up.name){
                  status = 3;
                }
                  ...
                  child: Container(
                    ...
                    child:  Padding(
                      ...
                      child: Row( crossAxisAlignment: CrossAxisAlignment.center, children: [
                        ...
                        Center(
                          child: SizedBox(
                            height: orderStatus == OrderStatusType.pending.name ? 50 : 60, width: orderStatus == OrderStatusType.pending.name ? 50 : 60,
                            child: Padding(
                              padding: const EdgeInsets.all(8.0),
                              child: Image.asset( status == 2 ? orderStatus == OrderStatusType.confirmed.name || orderStatus == OrderStatusType.accepted.name ? Images.processingGif
                                  : Images.cookingGif : status == 3
                                  ? orderStatus == OrderStatusType.handover.name ? Images.handoverGif : Images.onTheWayGif : Images.pendingGif,
